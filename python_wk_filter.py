...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  5 19:09:23 2024
...
class wk_analysis(object):
    ...
    def import_netcdf(self,file,varname,latname='latitude',latBound=15,time_range=['1997-01-01','2014-12-31'],model_name=None):
        if (self.data_status=="Empty"):
            ...
            ds = xr.open_dataset(file)[varname]
            if model_name!='GPCP':
                ds=ds*86400
            
            # ds['time'] = ds['time'].to_datetimeindex() 
            print(ds)
            f = ds.sel(time=slice(time_range[0],time_range[1])).interpolate_na(dim='time',method="linear")
            array_out = ds
            latitudes = f[latname][:]
            lat_ind = np.where(np.logical_and(latitudes>=-latBound,latitudes<=latBound))[0]
            array_out = array_out[:,lat_ind,:].data
    ...
    def plot_dual_background_removed(self,figsize=(18,8),text_size=12,contour_range=(0,2,0.1),min_contour_range_lines=1.1,\
            ...
            axAS[0].set_title('Anti-Symmetric/Background',size=text_size,fontweight='bold')
            axAS[1].set_title('Symmetric/Background',size=text_size,fontweight='bold')
            ...
            plt.colorbar(cset_0,ax=[axAS[0],axAS[1]],orientation='horizontal',
                         extend='neither',
            ...
            # plt.colorbar(cset_1,ax=axAS[1],orientation='horizontal')
    ...
    def export_data(self,name=None):
        ...
        name = name+'.wk_spectra.pkl'
        # np.save(name,self.wk_spectra)
        import pickle
        with open(name,'wb') as file:
            pickle.dump(self.wk_spectra,file)
    ...
    def save_figs(self,name=None,ext='png'):
        if (self.plots_available == "Yes"):
            ...
            for key in self.plots:
                self.plots[key].savefig(name+'_'+key+'.'+ext, bbox_inches='tight',dpi=300)
        else:
...
def wk_cmip(file, vari, cmip_model_name):
    """
    Perform Wheeler-Kiladis spectral analysis on the provided CMIP6 model data.

    Parameters:
        file (str): Path to the NetCDF file.
        vari (str): Variable name in the NetCDF file (e.g., 'precipitation' or 'pr').
        cmip_model_name (str): Name of the CMIP6 model being analyzed.
    """
    
    # Initialize the analysis with a specific name
    wk = wk_analysis(name="my_analysis")
    
    # Define the time range for analysis
    time_range = ['1997', '2014']
    timesta,timend =  '1997','2014'
    # Import the data from the NetCDF file
    wk.import_netcdf(file=file, varname=vari, latname="lat", latBound=15, 
                     time_range=time_range, model_name=cmip_model_name)
    
    # Set parameters for Wheeler-Kiladis spectral analysis
    spd = 1         # Sampling period
    nDayWin = 96    # Number of days in the window
    nDaySkip = 30   # Number of days to skip
    max_freq = 0.5  # Maximum frequency
    max_wn = 20     # Maximum wave number
    
    # Perform the Wheeler-Kiladis spectral analysis
    wa = wk.wheeler_kiladis_spectra(spd, nDayWin, nDaySkip, max_freq, max_wn)
    cmap = cmaps.ncl_default
    
    wk.plot_raw(figsize=(10, 8), text_size=12, contour_range=(-1.9, -0.15, 0.1), \
                    cmap='Reds', max_freq_plot=0.5, max_wn_plot=15, freq_lines=True, \
                    cpd_lines=[3, 6, 30])
    
    
    
    
    wk.plot_dual_raw(figsize=(18,8),text_size=12,contour_range=(-1.9,-0.15,0.1),\
                        cmap='Reds',max_freq_plot=0.8,max_wn_plot=20,freq_lines=True,\
                        cpd_lines=[3,6,30])
    
    
    wk.plot_background_removed(figsize=(10,8),text_size=12,contour_range=(0,2,0.1),min_contour_range_lines=1.1,\
                              matsuno_lines=True,he=[8,25,90],meridional_modes=[1],cmap=cmap,max_freq_plot=None,\
                      max_wn_plot=None,freq_lines=True,cpd_lines=[3,6,30],labels=True)
    
    
    wk.plot_dual_background_removed(figsize=(24,16),text_size=18,contour_range=(0,2,0.1),min_contour_range_lines=1.1,\
                                matsuno_lines=True,he=[8,25,90],meridional_modes=[1],cmap=cmap,max_freq_plot=None,\
                                max_wn_plot=None,freq_lines=True,cpd_lines=[3,6,30],labels=True)
    ...
    wk.export_data(cmip_model_name+'_daily_18_years_'+timesta+'-'+timend ) 
        
    wk.save_figs(cmip_model_name+timesta+'-'+timend) 
    return wa


# Define the output path for storing results
outpath = r'/Users/xpji/cmip6_wk/shell_code/'

# Create the output directory if it doesn't exist
if not os.path.exists(outpath):
    os.makedirs(outpath)
    print(f'{outpath} has been created')
else:
    print(f'{outpath} already exists')


# Define the path to the CMIP6 data file
cmip6_files = "/Datadisk/CMIP6_daily2/pr/stand_pr/pr_day_GPCP_data_1997-2014_stand_regrid.nc"

# Extract the base filename from the file path
i = os.path.basename(cmip6_files)
print(i)

# Determine the variable name based on the model type
if i.split('_')[2] == 'GPCP':
    varname = 'precipitation'
else:
    varname = 'pr'

# Extract the variable and model name for analysis
_, vari, cmip_model_name = cmip6_files, varname, i.split('_')[2]

# Run the Wheeler-Kiladis analysis
wk_cmip(cmip6_files, vari, cmip_model_name)
print(i, 'Wheeler-Kiladis analysis finished')
